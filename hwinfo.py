...
@author: shell.xu
'''
import os, re, sys, json, getopt, subprocess
...
def split_reader(src, sep, keys, stopblank=False):
    for line in src:
        if stopblank and not line: return
        r = line.split(sep, 1)
        if len(r) == 1: continue
        k, v = r[0].strip(), r[1].strip()
        if k.lower() in keys: yield k, v
...
def ipaddr():
    ...
    for line in check_output(['ip', 'addr']):
        if line.startswith(' '):
            r = line.strip().split()
            if r[0] == 'inet': ips.append(r[1])
            elif r[0] == 'link/ether': macs.append(r[1])
            continue
        if iface:
            try: unicode(iface)
            except UnicodeDecodeError:
                iface = 'Base64ed:' + base64.b64code(iface)
...
def hostname():
    with open('/etc/hostname') as fi: return [('Hostname', fi.read().strip())]

def run_info(funcs):
    rslt = {}
    for f in funcs:
        try:
            for k, v in f(): rslt.setdefault(k, []).append(v)
        except:
            import traceback
...
def all_info():
    return run_info([dmidecode, diskinfo, diskusage, ethinfo, memusage, hostname])
